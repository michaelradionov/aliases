...
echo 'Скрип Go Git Aliases. Первая версия. Git 1.X НЕ ПОДДЕРЖИВАЕТСЯ';
...
then
  ...
  # case $x in
  #   y)
      echo 'Не нашел маркеры Go Git Aliases: "### GGA_START" и "### GGA_END". Либо блока еще нет, либо сломаны оба маркера. Добавляю новый блок Go Git Алиасов в конец вашего ~/.bashrc...';
...
if [[ ! $BEGIN_LINE ]] || [[ ! $END_LINE ]]
then
  echo 'Похоже, что один из маркеров GGA сломан. Вам придется это починить самостоятельно. \nДолжно быть "### GGA_START" в начале и "### GGA_END" в конце. Самоудаляюсь.';
  rm -f ${0##*/}
  exit 0;
fi
...
then
  ...
  # case $x in
  #   y)
      echo 'Нашел блок Go Git Алиасов. Обновляю';
      # Заменяем блок
